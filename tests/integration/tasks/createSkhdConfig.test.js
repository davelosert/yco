...
const withoutIndentSpaces = multilineString => multilineString
  .replace(/^\s*/g, '')
  .replace(/(\n\s*)/g, '\n')
  .replace(/\n$/g, '');
...
suite('task: createSkhdConfig()', () => {
  ...
  test('writes info about file generation as comment to ~/.confg/yabai/yco.skhd.conf', async () => {
    ...
    assert.ok(ycoSkhdConfContent.includes('# This file was automatically generated by "yco create-configs". Changes will be overwritten.'));
  });
  ...
  suite('config: switchFocus', () => {
    test('creates switchFocus entries for every config', async () => {
      const { ycoSkhdConfPath } = await createTestDirFromFixture('firstRun');
